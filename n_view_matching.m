function point_matrix = n_view_matching(points, features, ima, MaxRatio, Metric, MatchThreshold)
...
% point_matrix: Point correspondences (non-hom coordinates) allong all views (in the same format of the lab sessions of Unit III) 
    ...
    for j = 2:numImages
        indexPairs       = matchFeatures(features{j-1},features{j},'MaxRatio',MaxRatio,'Metric',Metric, 'MatchThreshold', MatchThreshold);
...
%     indexPairs       = matchFeatures(features{1},features{6},'MaxRatio',MaxRatio,'Metric',Metric, 'MatchThreshold', MatchThreshold) ;
%     matchedPoints{1} = points{1}(indexPairs(:,1));
%     matchedPoints{2} = points{6}(indexPairs(:,2));
%     figure(numImages+1); showMatchedFeatures(ima{1},ima{6},matchedPoints{1},matchedPoints{2});
